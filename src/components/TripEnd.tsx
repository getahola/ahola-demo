...
export function TripEnd({ passengers, onDone }: { passengers: Passenger[]; onDone: (keptIds: string[]) => void }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h2 className="text-xl font-semibold text-slate-800">Your trip is ending</h2>
        <p className="text-sm text-slate-500">
          Your discovery list will be cleared. Choose which chats to keep — a chat only stays if{' '}
          <span className="font-medium text-slate-700">both people</span> agree.
        </p>
      </div>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs">
                  {status === 'idle' && <span className="text-slate-400">Keep this chat after the trip?</span>}
                  {status === 'pending' && <span className="text-amber-600">Waiting for {passenger.name}…</span>}
                  {status === 'kept' && (
      ...
      <Button onClick={() => onDone(keptIds)} disabled={anyPending}>
        {keptIds.length > 0
          ? `Done — keep ${keptIds.length} chat${keptIds.length > 1 ? 's' : ''}`
          : 'Done — clear all chats'}
      </Button>
    </div>
  )
}
